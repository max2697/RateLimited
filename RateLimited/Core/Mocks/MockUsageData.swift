...
enum MockUsageProvider {
    nonisolated static func codexSnapshot(now: Date = Date()) -> ToolUsageSnapshot {
        ToolUsageSnapshot(
            ...
            weekly: UsageWindow(usedPercent: 48, resetDate: now.addingTimeInterval(4 * 86_400 + 5 * 3600))
        )
    }
    ...
    nonisolated static func claudeSnapshot(now: Date = Date()) -> ToolUsageSnapshot {
        ToolUsageSnapshot(
            ...
            weekly: UsageWindow(usedPercent: 24, resetDate: now.addingTimeInterval(5 * 86_400 + 3600))
        )
    }
}
